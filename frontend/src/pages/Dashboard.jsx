        ...
        <div>
          ...
          <p className="text-gray-600 mt-2">
            ...
            <span className="font-semibold text-dark">
              {dashboardData?.user?.course || "Exams"}
            </span>
            ?
          </p>
        </div>
            ...
            <div className="flex items-baseline gap-2 mt-2">
              ...
              <span className="text-3xl font-bold text-primary">
                {dashboardData?.overallStats?.averagePercentage > 0
                  ? `${dashboardData.overallStats.averagePercentage}%`
                  : "No Data Available"}
              </span>
        ...
        <div className="bg-gradient-to-br bg-black text-white p-6 rounded-2xl shadow-lg flex flex-col justify-center col-span-2">
          <div className="flex justify-between items-start">
            ...
            <button
              ...
              className="bg-white text-black px-4 py-2 rounded-xl font-bold text-sm hover:bg-indigo-50 transition shadow-md whitespace-nowrap"
            >
              Chat Now
            </button>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-black text-white p-8 rounded-2xl shadow-lg flex items-center justify-between">
            <div>
              ...
              <p className="text-purple-100">
                Easy difficulty • Based on{" "}
                {dashboardData?.user?.course || "your course"}
              </p>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {dashboardData?.subjectAnalytics?.map((item) => (
              <div
                key={item.subject}
                className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm transition hover:shadow-md group"
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="text-xs text-gray-400">
                    {item.totalTests} Tests Taken
                  </span>
                </div>
                <h4 className="font-bold text-gray-800 group-hover:text-primary transition">
                  {item.subject}
                </h4>
                <div className="mt-4">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-fit">
          ...
          {loadingTests ? (
            ...
            <div className="text-center py-8 text-gray-400 text-sm">
              No history found.{" "}
              <Link to="/quiz" className="text-primary font-bold">
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h4 className="font-bold text-gray-800 text-sm line-clamp-1 group-hover:text-primary transition">
                          {test.title}
                        </h4>
                      ...
                      <span
                        className={`text-[10px] font-bold px-2 py-1 rounded-full ${isPass ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"}`}
                      >
                        {percentage}%
                      </span>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-xl bg-white h-full shadow-2xl overflow-y-auto animate-slide-in p-6 flex flex-col">
            ...
            <div className="bg-indigo-50 p-6 rounded-2xl flex items-center justify-between mb-8">
              ...
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center font-bold text-white ${
                  selectedTest.score / selectedTest.total >= 0.7
                    ? "bg-green-500"
                    : "bg-orange-500"
                }`}
              >
            ...
            <h3 className="font-bold text-dark text-lg mb-4 flex items-center gap-2">
              <BarChart2 size={20} className="text-primary" /> Question
              Breakdown
            </h3>
            ...
            <div className="space-y-4 pb-10">
              {selectedTest.questions?.map((q, idx) => (
                <div
                  key={idx}
                  className={`p-4 rounded-xl border-l-4 shadow-sm bg-white border border-gray-100 ${
                  ...
                  <div className="grid grid-cols-1 gap-2 mt-2">
                    ...
                    {!q.isCorrect && (
                      <div className="p-2 bg-red-50 border border-red-100 rounded-lg text-xs text-red-700">
                        <span className="font-bold">You chose:</span>{" "}
                        {q.userAnswer}
                      </div>
                    )}
                  </div>
